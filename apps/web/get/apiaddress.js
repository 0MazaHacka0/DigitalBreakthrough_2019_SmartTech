module.exports.func = async (con, req, res) => {
	...
	let data = await con.promise(con, con.query, 'SELECT name, address, lat, lng FROM addressCompanies WHERE id = (SELECT id FROM users WHERE keyg = ?)', [req.headers['authorization'].split(' ')[1]]);
	data = data.res;
